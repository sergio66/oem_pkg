...
elseif driver.topts.dataset == 30  
  junkcov = zeros(length(xb),length(xb));
  junkind = amsu_scalar_i; junkcov(junkind,junkind) = driver.oem.cov(6,6);  %% stemp
  junkind = amsu_water_i;  junkcov(junkind,junkind) = driver.oem.cov(driver.jacobian.water_i,driver.jacobian.water_i);  %% WV
  junkind = amsu_temp_i;   junkcov(junkind,junkind) = driver.oem.cov(driver.jacobian.temp_i, driver.jacobian.temp_i);   %% TZ
  ...
  elseif invtype == 1
    rcov = pinv(junkcov);
  elseif invtype == 2
    rcov = invillco(junkcov);
  elseif invtype == 3
    rcovF = factorize(junkcov);
    rcov  = inverse(junkcov);
    rcov  = rcov * eye(size(rcov));
  elseif invtype == 4
    rcov = inverse_ridge_regression_matrix(junkcov,kmax);
  elseif invtype == 5
    rcov = inverse_minimum_eigenvalue_matrix_optim(junkcov,kmaxrange,sigminrange,'junkcov');
  end
  ...
  lenS = length(driver.jacobian.scalar_i);
  lenS = 1;
  %% default : only stemp jacs, layer WV
  rc = blkdiag(zeros(1,1),driver.oem.alpha_water*s);
  
  %% always assumes you want to fit for WV ... may want to keep T fixed 9so no fit) and also may not want to fit O3
  if isfield(driver.oem,'alpha_temp')
    rc = blkdiag(zeros(lenS,lenS),driver.oem.alpha_water*s,driver.oem.alpha_temp*s);
  elseif isfield(driver.oem,'alpha_temp') & ~isfield(driver.oem,'alpha_ozone')
    rc = blkdiag(zeros(lenS,lenS),driver.oem.alpha_water*s,driver.oem.alpha_temp*s);
  elseif ~isfield(driver.oem,'alpha_temp') & isfield(driver.oem,'alpha_ozone')
    error('no ozone in AMSU')
    rc = blkdiag(zeros(lenS,lenS),driver.oem.alpha_water*s,driver.oem.alpha_ozone*s);
  end
end
...
%{
get_inv_se_rcov_allchans_allparams_notused
%}
